...
class RoomService {
  ...
  async getRoomSuggestions(
    ...
    const params = {
      ...
      ...Object.fromEntries(
        Object.entries(filters).filter(
          ([key, value]) => value !== null && value !== "" && key !== "room", // Виключаємо поточну кімнату з параметрів
        ),
      ),
    };

    console.log(
      "Room suggestions request params:",
    ...
    console.log(
      "Room suggestions response:",
      JSON.stringify(suggestedRooms, null, 2),
    );

    // Якщо вказано фільтр зайнятості, фільтруємо додатково по зайнятості
    if (filters.busy !== null) {
      const busyRoomsResponse = await httpClient.get("/rooms/busy_rooms/", {
        params,
      });
      ...
      if (filters.busy === false) {
        ...
        return suggestedRooms.filter(
          (room: string) => !busyRooms.includes(room),
        );
      } else {
        // Повертаємо тільки зайняті аудиторії зі списку рекомендованих
        return suggestedRooms.filter((room: string) =>
          busyRooms.includes(room),
        );
      }
    }
